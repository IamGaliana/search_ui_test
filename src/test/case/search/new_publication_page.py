...
class NewPublicationPage(unittest.TestCase):
    ...
    def test11_check_product_bigpic(self):
        ...
        self.searchcommon.product_area_check('pub', 'big_pic')

    def test12_check_product_list(self):
        log_info('test case 12: check products')
        self.searchcommon.product_area_check('pub', 'list')
...
if __name__ == '__main__':
    query_list = BasicFunc().get_query_list("new_publish")
    for query in query_list:
        log_info("QUERY - %s" % query)
        QUERY_WORD = query
        suit = unittest.defaultTestLoader.loadTestsFromTestCase(NewPublicationPage)
