...
class VerticalMallPage(unittest.TestCase):
    ...
    def test02_check_vertical_area(self):
        ...
        self.searchcommon.vertical_search_check('mall')
    ...
    def test12_check_product_bigpic(self):
        ...
        self.searchcommon.product_area_check('mall', 'big_pic')

    def test13_check_product_list(self):
        log_info('---------- check products ----------')
        self.searchcommon.product_area_check('mall', 'list')
...
if __name__ == '__main__':
    query_list = BasicFunc().get_query_list("vertical_mall")
    for query in query_list:
        QUERY_WORD = query
        log_info("QUERY - %s" % query)
        suit = unittest.defaultTestLoader.loadTestsFromTestCase(VerticalMallPage)
